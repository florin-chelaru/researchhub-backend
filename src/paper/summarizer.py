    ...
    def get_summary(self, openai_model: str = "text-davinci-003", max_response_tokens: int = 120) -> Optional[str]:
        ...
        try:
            ...
            if paragraphs is None or len(paragraphs) == 0:
                logger.info(f"Unable to extract paragraphs from paper {self._paper.id}.")
                return None
            ...
            for chunk in chunks:
                ...
                if summary_part != "":
                    ...
                    logger.info(f"Summary for chunk: {summary_part}")

            if len(summary_parts) == 0:
                logger.info(f"Failed to extract OpenAI summary for the paper {self._paper.id}")
                return None
